...
def test_tiny_file_produces_zero_chunks_but_is_indexed(
    ...
    # And rust is *present* in the language inventory with nothing live in it.
    # Keyed on chunks alone this language had no rows at all and so was absent —
    # "indexed but unsearchable" read exactly like "this project has no rust".
    assert s["languages"]["rust"]["files"] == 1, s
    assert s["languages"]["rust"]["indexed_files"] == 1, s
    assert s["languages"]["rust"]["chunks_active"] == 0, s
