...
def _rust_chunks(client: httpx.Client, project: str) -> dict:
    """The rust chunk counts from the language inventory, defaulting to zeros."""
    return (
        stats(client, project)
        .json()["languages"]
        .get("rust", {"chunks_active": 0, "chunks_deleted": 0})
    )
...
def _baseline_active_chunks(client: httpx.Client, code: str) -> int:
    ...
    return _rust_chunks(client, ctrl)["chunks_active"]
...
def test_concurrent_same_file_no_chunk_leak(
    ...
    assert chunks["chunks_active"] == baseline, chunks
...
def test_concurrent_reindex_changing_content_settles(
    ...
    assert chunks["chunks_active"] in (base_v1, base_v2), chunks
    assert chunks.get("deleted", 0) == 0, chunks
    assert search(client, project, "process records").status_code == 200
...
def test_concurrent_index_and_delete_files(
    ...
    assert final["languages"].get("rust", {}).get("chunks_active", 0) == 0, final
...
def test_failed_index_does_not_clobber_a_later_index(
    ...
    run_gc(client)
    chunks = _rust_chunks(client, project)
    assert chunks["chunks_active"] >= 1, chunks
...
def _assert_settled(client: httpx.Client, project: str) -> None:
    ...
    for lang in body["languages"].values():
        assert lang.get("chunks_deleted", 0) == 0, body
