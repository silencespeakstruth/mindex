...
Integration tests for the management endpoints:
  ...
  GET    /projects/{guid}          — stats (files by status + per-language inventory)
...
def test_stats_aggregates_by_status_and_language(
    ...
    assert body["languages"]["rust"]["chunks_active"] >= 1
    assert body["languages"]["python"]["chunks_active"] >= 1
    assert body["languages"]["rust"]["chunks_deleted"] == 0
    # The file half of the inventory: one file per language, both indexed.
    assert body["languages"]["rust"]["files"] == 1, body
    assert body["languages"]["rust"]["indexed_files"] == 1, body
    assert body["languages"]["python"]["files"] == 1, body
    assert body["languages"]["python"]["indexed_files"] == 1, body
...
def test_reindex_shows_deleted_chunks_until_gc(
    ...
    assert before["languages"]["rust"]["chunks_deleted"] >= 1, before
    assert before["languages"]["rust"]["chunks_active"] >= 1, before

    assert run_gc(client).status_code == 200

    after = stats(client, project).json()
    assert after["languages"]["rust"]["chunks_deleted"] == 0, after
    assert after["languages"]["rust"]["chunks_active"] >= 1, after
...
def test_delete_files_by_path_glob_then_gc(client: httpx.Client, project: str) -> None:
    ...
    assert before["languages"]["rust"]["chunks_deleted"] >= 1, before
    # Both files are still tracked; only one of them is still indexed.
    assert before["languages"]["rust"]["files"] == 2, before
    assert before["languages"]["rust"]["indexed_files"] == 1, before
    ...
    assert after["languages"]["rust"]["chunks_deleted"] == 0, after
    assert after["languages"]["rust"]["files"] == 1, after
...
def test_delete_files_by_language(client: httpx.Client, project: str) -> None:
    ...
    run_gc(client)
    after = stats(client, project).json()
    # GC prunes the emptied `deleted` file row too, so python may leave the
    # inventory entirely — but if it is still there it must have nothing live.
    assert (
        "python" not in after["languages"]
        or after["languages"]["python"]["chunks_active"] == 0
    ), after
    assert after["languages"]["rust"]["chunks_active"] >= 1, after
