...
# ─── Accelerator backend ─────────────────────────────────────────────────────
# Two backends are in use on this host, one per systemd instance (see
# embedder/systemd/): AMD via ROCm, where torch.cuda.* maps to HIP and the device
# string is still "cuda", and the Intel iGPU via torch's XPU backend ("xpu").
# They differ only in which submodule owns empty_cache(), so resolve it once
# instead of testing the device string at each call site.


def accelerator(device: str):
    """The torch submodule owning `device`'s memory, or None for CPU/unknown."""
...
def default_device() -> str:
    """Best available device, checked in the order this host cares about."""
...
@contextmanager
def attention_backend(device: str) -> Iterator[None]:
    """Keep XPU off its own fused attention kernel.

    On Intel XPU (torch 2.13, Xe2/Arrow Lake-P) scaled_dot_product_attention's
    default backend returns **NaN** for any row that carries padding, in fp16 and
    with an attention mask — i.e. for every batch of more than one text, since
    they get padded to the longest. It is silent: the NaN lands in the padded
    rows' dense/sparse/ColBERT heads and the request still returns 200. Measured
    against the same weights on CPU and on ROCm, a single unpadded text agrees to
    cosine 0.999997 while a two-text batch collapses to 0.265.

    EFFICIENT_ATTENTION and MATH both compute it correctly; both are listed, in
    that order, so the cheaper one is used where it applies and MATH catches the
    shapes it declines. The cost is roughly 2x the (wrong) default's time.

    Non-XPU devices are left alone — this is a workaround for one backend, not a
    policy, and pinning ROCm's choice would forfeit its flash kernel. Re-test on
    torch upgrades: when the default is fixed, this whole thing goes away.
    """
...
def encode_direct(
    ...
    while i < len(items_sorted):
        ...
        try:
            ...
            # torch.OutOfMemoryError is the backend-neutral base class (torch ≥ 2.5);
            # torch.cuda.OutOfMemoryError is an alias of it, and XPU raises the same.
            is_oom = (
...
parser.add_argument(
    ...
    help="Single torch device for the model: 'cuda' / 'cuda:N' (NVIDIA or AMD via "
    "ROCm), 'xpu' / 'xpu:N' (Intel), or 'cpu'. Pin a specific card with the index "
    "(or HIP_VISIBLE_DEVICES / ONEAPI_DEVICE_SELECTOR). Passed to FlagEmbedding as "
    "`devices=` — one device, no multi-process pool.",
)
...
class ModelManager:
    ...
    def _drop(self) -> None:
        ...
        self._model = None
        gc.collect()
        accel = accelerator(self._device)
        if accel is not None:
            accel.empty_cache()
    ...
    def _run(self, work: Callable[[BGEM3FlagModel], Any]) -> Any:
        ...
        poisoned accelerator context is rebuilt on the next request."""
