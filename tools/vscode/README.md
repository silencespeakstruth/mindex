...
- **Ask** (sidebar → Ask). One box for both ways of querying the index, with a
  ...
  Shift+Enter adds a newline. Where the answers land differs, because the two answers
  ...
  - **Search** — a QuickPick in rank order (`#rank score path`, line span, snippet)
    ...
    query so you never have to reach for the sidebar.
  - **Research** — a **local** model does the whole investigation; steps, its live
    ...
    produced. *Cancel* — or closing the tab — drops the connection, which **is** the
    server-side cancel. One run at a time.
...
- **Server Status + status bar.** `/health` and `/status`, plus the failed-file
  dead-letter list with per-file or all-at-once *Retry*. The `ollama` check is the
  server's **optional** dependency: when it is down, Health stays `ok` (indexing and
  search are unaffected) and only Research breaks — so it renders as a warning, the
  status bar appends `research`, and the Ask view's Research tab shows a notice. A run
  that fails on it re-checks health, so the two agree without a manual refresh.
...
```sh
code --install-extension mindex-vscode-1.0.0.vsix   # then: Developer: Reload Window
```
...
ln -s "$PWD" ~/.vscode/extensions/mindex.mindex-vscode-1.0.0
```
...
| `mindex.topK` | `10` | Search results to request |
