        ...
        vscode.window.withProgress(
            {
                ...
                title: force ? "mindex: force reindexing" : "mindex: reindexing",
                cancellable: true,
            },
            (progress, token) =>
                doReindex(api, guid, root, relPaths, batchSize, progress, token, force)
        );
    try {
        return await run();
    } catch (e) {
        ...
        await reportError(`Reindex of ${relPaths.length} file(s) failed`, e, async () => {
            retried = await reindexPaths(api, guid, root, relPaths, batchSize, force);
        });
        return retried;
    }
}
