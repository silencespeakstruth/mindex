...
Options:
  ...
  --project GUID          project GUID from .mindex, dashed or not (required)
...
# Accept either spelling — the server parses the path segment as a UUID, and the
# .mindex files people copy their GUID from carry the dashed form. Normalizing to
# lowercase dashless keeps one shape on the wire.
PROJECT="${PROJECT,,}"
if [[ "$PROJECT" =~ ^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$ ]]; then
    PROJECT="${PROJECT//-/}"
fi
[[ "$PROJECT" =~ ^[0-9a-f]{32}$ ]] || die "--project must be a project GUID, dashed or not, got: $PROJECT"
