...
- **A repo-root `.mindex`** — the hook does not parse it: `mindex-index` reads the
  GUID and the `exclude_paths` scope itself (the hook only asks it for the GUID,
  via `--print-guid`, to address the delete endpoint). So the reindex is scoped
  exactly like a manual run, and paths the project deliberately excludes stay out.
