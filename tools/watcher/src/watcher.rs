...
mod tests {
    ...
    fn classify_language_filter_applies_to_upserts_only() {
        ...
        assert!(classify_upsert("/repo/notes.txt", &[], &[], &[]).is_none());
        // Documentation is a language, so it is watched like any other file.
        assert!(classify_upsert("/repo/README.md", &[], &[], &[]).is_some());
        assert!(classify_upsert("/repo/README.md", &[], &[], &["rust"]).is_none());
    }
