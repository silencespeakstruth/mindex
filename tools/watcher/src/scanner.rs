...
mod tests {
    ...
    fn unknown_or_missing_extension_detects_nothing() {
        ...
        for path in ["notes.txt", "Makefile", "a.rs.bak", ".gitignore"] {
            assert_eq!(detect_language(Path::new(path)), None, "{path}");
        }
    }

    /// This map is a verbatim copy of the indexer's; a language missing here and
    /// present there makes the watcher silently blind to it.
    #[test]
    fn documentation_is_detected() {
        for path in ["README.md", "docs/guide.markdown"] {
            assert_eq!(
