...
\n\
The project root must contain a .mindex marker file: YAML with a `guid:` key.\n\
Optional include_paths, exclude_paths and languages lists in .mindex narrow\n\
...
struct Cli {
    ...
    /// value is visible in `ps` to every user on the machine, and this process is
    /// long-lived.
    ///
...
async fn main() -> Result<()> {
    ...
    let mf = mindexfile::parse(&root.join(mindexfile::FILE_NAME))?;
    let (include_set, exclude_set) =
        mindexfile::build_globsets(&mf.include_paths, &mf.exclude_paths)?;

    let http = Arc::new(build_http_client(&wcfg).context("failed to build HTTP client")?);
...
/// True when `abs` is larger than one `code` value may be (`MAX_CODE_BYTES`).
///
/// A file whose size cannot be read is *not* treated as over-cap: the read that
/// follows will fail on its own and drop it, which is the same outcome by a route
/// that does not guess.
async fn over_cap(abs: &Path) -> bool {
    match tokio::fs::metadata(abs).await {
        Ok(m) => m.len() > mindexfile::MAX_CODE_BYTES,
        Err(_) => false,
    }
}
...
fn build_http_client(cfg: &config::WatcherConfig) -> Result<reqwest::Client> {
...
/// Renders an `anyhow` error together with **all** its causes on one line.
///
/// A bare `%e` prints only the outermost context, so a request that failed on
/// `invalid peer certificate: UnknownIssuer` logs as nothing but `POST <url>` —
/// which reads as "the server is down" and sends you looking in the wrong place.
/// Use this for every error that crossed a `Context` boundary.
...
async fn flush(
    ...
    for event in events {
        ...
        match event {
            ...
            PendingEvent::Upsert { rel, lang } => {
                ...
                if over_cap(&abs).await {
                    warn!(file = %rel, "file is over the size cap, skipping");
                    continue;
                }
    ...
    if n_upsert > 0 {
        ...
        .await
        {
            ...
            Err(e) => warn!(
                error = %chain(&e),
                "index_batch failed; drift will correct on next check"
            ),
        }
    }
    ...
    if !deletes.is_empty() {
        ...
        match client::delete_files(http, &cfg.server_url, &cfg.guid, deletes, cancel).await {
            ...
            Err(e) => warn!(
                error = %chain(&e),
                "delete_files failed; drift will correct on next check"
            ),
        }
    }
}
...
async fn run_drift(cfg: &Cfg, http: &reqwest::Client, cancel: &CancellationToken) {
    ...
    let drift = match client::check_drift(http, &cfg.server_url, &cfg.guid, manifest, cancel).await
    {
        ...
        Err(e) => {
            warn!(
                error = %chain(&e),
                "drift check failed; the index is left as it is until the next check. \
                 Check the server is up, the URL and project GUID are right, and that \
                 its TLS certificate is issued by a CA this host trusts (--no-verify \
                 skips that check)."
            );
            return;
        }
    ...
    if !drift.orphaned.is_empty() {
        ...
        match client::delete_files(http, &cfg.server_url, &cfg.guid, drift.orphaned, cancel).await {
            ...
            Err(e) => warn!(error = %chain(&e), "drift: delete orphaned files failed"),
        }
    }
}
...
async fn build_manifest(cfg: &Cfg, cancel: &CancellationToken) -> HashMap<String, String> {
        ...
        .filter(|e| e.file_type().is_file())
    {
        ...
        if over_cap(&abs).await {
            // Too big for one `code` value: the server would reject the batch it
            // travelled in, so claiming it here would report it `missing` forever.
            continue;
        }
...
async fn reindex_files(
    ...
    .await
    {
        ...
        Err(e) => warn!(error = %chain(&e), "drift: reindex failed"),
    }
}
