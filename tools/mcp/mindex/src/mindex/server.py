...
def drift(
    ...
    # is the one tool that fails on a deployment where the others work. TLS is
    # forwarded explicitly below; the credential is not, because `mindex-index`
    # reads ``MINDEX_TOKEN`` from the environment it inherits from this process.
    # That inheritance is load-bearing: adding an ``env=`` to the run() call below
    # would silently break authorization for this tool alone. Do not pass the
    # token as ``--token`` instead — a flag value is visible in `ps`.
    verify = _verify()
    if verify is False:
